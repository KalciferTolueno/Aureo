...
<style scoped>
.journey-map{position:relative;grid-column:1/-1;isolation:isolate;overflow:hidden;border:1px solid rgba(125,167,151,.24);border-radius: var(--radio-organico-2);background:#090e15;box-shadow:0 24px 58px rgba(0,0,0,.22)}
.journey-map-canvas{height:clamp(22rem,48svh,34rem);background:#090e15}
.journey-empty{position:absolute;z-index:500;left:50%;bottom:2rem;width:min(90%,32rem);margin:0;padding:.6rem 1rem;border-radius: var(--radio-pill);background:rgba(8,11,17,.78);color:#c5d5cd;font-style:italic;text-align:center;transform:translateX(-50%);pointer-events:none}
...
:deep(.leaflet-popup-content strong){color:#ead6a7;font-size: var(--texto-4);font-weight:400}
...
@media(max-width:560px){.journey-map-canvas{height:min(28rem,60svh)}.journey-empty{bottom:2.5rem;font-size: var(--texto-3)}}
</style>