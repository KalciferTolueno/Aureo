...
const total = computed(() => baseIncome.value + movements.items.value.reduce((sum, item) => sum + (item.tipo === 'ingreso' ? item.monto : -item.monto), 0))
const currency = new Intl.NumberFormat('es-CL', { style: 'currency', currency: 'CLP', maximumFractionDigits: 0 })
const selectedBalanceFlower = ref<string | null>(null)
const blossomAnchors = [
  [18, 42], [25, 30], [31, 47], [36, 25], [41, 38], [46, 18], [51, 31], [56, 44],
  [60, 22], [65, 34], [70, 16], [74, 29], [79, 42], [84, 25], [88, 38], [22, 55],
  [33, 59], [44, 53], [55, 60], [66, 52], [76, 58], [87, 53], [48, 69], [68, 68],
] as const
const balanceFlowers = computed(() => [...movements.items.value]
  .filter((item) => item.tipo === 'gasto')
  .reverse()
  .slice(0, blossomAnchors.length)
  .map((item, index) => {
    const anchor = blossomAnchors[index] ?? blossomAnchors[0]
    return {
      item,
      x: anchor[0],
      y: anchor[1],
      scale: .82 + (index % 4) * .09,
      rotation: -18 + (index * 23) % 37,
      delay: Math.min(index * 45, 720),
    }
  }))
const currentBalanceFlower = computed(() => balanceFlowers.value.find(({ item }) => item.id === selectedBalanceFlower.value)?.item ?? null)
const balanceDate = (value: string) => new Intl.DateTimeFormat('es-CL', { day: 'numeric', month: 'long' }).format(new Date(value))
async function toggleBalance() { hiddenBalance.value = !hiddenBalance.value; await storage.set('balance_oculto', hiddenBalance.value) }
...
const categoryTotals = computed(() => Object.fromEntries(categories.map((category) => [category, movements.items.value.filter((item) => item.tipo === 'gasto' && item.categoria === category).reduce((sum, item) => sum + item.monto, 0)])))
const maxCategoryTotal = computed(() => Math.max(1, ...Object.values(categoryTotals.value)))
function branchStyle(index: number) {
  const amount = categoryTotals.value[categories[index]!] ?? 0
  return { opacity: .32 + (amount / maxCategoryTotal.value) * .68, strokeWidth: 7 + (amount / maxCategoryTotal.value) * 9 }
}
...
onMounted(async () => {
  hiddenBalance.value = (await storage.get<boolean>('balance_oculto')) ?? true
  ...
  <section class="tw-workspace" :class="[`detail-${detail}`, { 'is-contemplative': contemplative }]" :aria-label="title">
    ...
    <div v-else-if="isWorld" class="workspace-grid" :class="{ 'constellation-workspace': detail === 'world-vinculos' }">
      <section v-if="detail === 'world-vinculos'" class="constellation-space" aria-label="Mapa de Mi Constelación">
        <p class="constellation-copy">Cada vínculo encuentra su distancia: lo íntimo al centro, lo que sostiene en medio y lo que orienta en el horizonte.</p>
    ...
    <div v-else-if="detail === 'balance'" class="balance-workspace">
      <section class="balance-reading">
        <div class="balance-heading">
          <button type="button" :aria-label="hiddenBalance ? 'Mostrar balance' : 'Ocultar balance'" @click="toggleBalance"><span>Lo que tengo hoy</span><strong>{{ hiddenBalance ? '· · · · ·' : currency.format(total) }}</strong></button>
          <p>Cada gasto abre una flor. Al tocarla, puedes recordar en qué elegiste usar tus recursos.</p>
        </div>
        <div class="balance-actions"><button type="button" :class="{ active: balanceMode === 'movement' }" @click="balanceMode = balanceMode === 'movement' ? 'overview' : 'movement'">Registrar movimiento</button><button type="button" :class="{ active: balanceMode === 'goal' }" @click="balanceMode = balanceMode === 'goal' ? 'overview' : 'goal'">Nueva meta</button></div>
      </section>
      ...
      <section class="balance-grove" aria-label="Cerezo de gastos" @click="selectedBalanceFlower = null">
        <svg class="cherry-tree" viewBox="0 0 720 560" role="img" aria-label="Árbol de cerezo cuyas flores representan tus gastos">
          <defs>
            <linearGradient id="balance-trunk" x1="0" y1="0" x2="1" y2="1"><stop stop-color="#32241f" /><stop offset=".52" stop-color="#8c6653" /><stop offset="1" stop-color="#c09a7d" /></linearGradient>
            <linearGradient id="balance-branch" x1="0" y1="1" x2="1" y2="0"><stop stop-color="#5b4036" /><stop offset="1" stop-color="#b88c70" /></linearGradient>
            <filter id="balance-tree-depth" x="-20%" y="-20%" width="140%" height="160%"><feDropShadow dx="0" dy="18" stdDeviation="16" flood-color="#000" flood-opacity=".38" /></filter>
          </defs>
          <ellipse class="cherry-ground" cx="366" cy="522" rx="180" ry="18" />
          <g filter="url(#balance-tree-depth)">
            <path class="cherry-trunk" d="M328 516C339 462 333 409 349 352C365 296 372 253 356 202C348 176 356 151 371 126C363 180 394 205 389 257C384 309 369 347 382 399C394 447 391 487 407 516Z" />
            <path class="cherry-trunk-light" d="M356 493C359 430 350 385 367 334C384 281 386 231 371 184C393 225 398 273 382 326C366 378 382 435 385 493Z" />
            <path class="cherry-branch branch-one" :style="branchStyle(0)" d="M366 319C322 278 282 257 223 246C168 236 130 208 97 171" />
            <path class="cherry-branch branch-two" :style="branchStyle(1)" d="M361 271C313 235 287 199 259 154C241 124 211 102 169 87" />
            <path class="cherry-branch branch-three" :style="branchStyle(2)" d="M376 284C424 235 463 216 518 203C567 191 608 160 644 120" />
            <path class="cherry-branch branch-four" :style="branchStyle(3)" d="M381 346C431 311 480 302 537 310C589 317 626 298 668 265" />
            <path class="cherry-branch branch-five" :style="branchStyle(4)" d="M371 226C406 181 428 142 446 91" />
            <path class="cherry-twig" d="M222 246L179 293M283 201L307 150M518 203L548 151M537 310L572 354M446 91L499 69M169 87L122 103" />
            <path class="bonsai-pot" d="M291 503h150l-13 42H304Z" /><path class="bonsai-pot-rim" d="M282 494h168v16H282Z" />
          </g>
        </svg>
        <span class="cherry-moon" aria-hidden="true" />
        <button
          v-for="flower in balanceFlowers"
          :key="flower.item.id"
          type="button"
          class="balance-blossom"
          :class="{ active: selectedBalanceFlower === flower.item.id }"
          :style="{ left: `${flower.x}%`, top: `${flower.y}%`, '--blossom-scale': flower.scale, '--blossom-rotation': `${flower.rotation}deg`, '--blossom-delay': `${flower.delay}ms` }"
          :aria-label="`${flower.item.nota || flower.item.categoria}: ${currency.format(flower.item.monto)}`"
          :aria-pressed="selectedBalanceFlower === flower.item.id"
          @click.stop="selectedBalanceFlower = selectedBalanceFlower === flower.item.id ? null : flower.item.id"
        ><span class="balance-blossom-shape" aria-hidden="true"><i v-for="petal in 5" :key="petal" :style="{ '--petal': petal }" /><b /></span></button>
        <Transition name="balance-flower-reading">
          <article v-if="currentBalanceFlower" class="balance-flower-card" role="status" @click.stop>
            <button type="button" aria-label="Cerrar detalle del gasto" @click="selectedBalanceFlower = null"><AppIcon name="close" /></button>
            <span>{{ currentBalanceFlower.categoria }}</span>
            <strong>{{ currency.format(currentBalanceFlower.monto) }}</strong>
            <p>{{ currentBalanceFlower.nota || 'Un movimiento que ya forma parte de tu historia.' }}</p>
            <time :datetime="currentBalanceFlower.fecha">{{ balanceDate(currentBalanceFlower.fecha) }}</time>
          </article>
        </Transition>
        <div v-if="!balanceFlowers.length" class="balance-grove-empty"><AppIcon name="balance" /><p>Tu cerezo espera su primera flor.<br />Los gastos que registres aparecerán aquí.</p></div>
        <p v-else class="balance-grove-note">{{ balanceFlowers.length }} {{ balanceFlowers.length === 1 ? 'flor guarda' : 'flores guardan' }} tus movimientos más recientes</p>
      </section>
      ...
      <div class="golden-presence">
        <div class="golden-object-column">
          <section ref="darumaStage" class="golden-sculpture daruma-stage" :class="{ 'daruma-paused': !darumaActive }" aria-label="Daruma de kintsugi de tu Edad Dorada">
            ...
            <svg class="daruma-art" viewBox="0 0 360 450" role="img" aria-label="Daruma con grietas de oro que representan tus declaraciones">
              ...
              <g class="daruma-form" filter="url(#daruma-depth)">
                ...
                <path class="daruma-moustache" d="M178 171C159 159 137 165 122 183C143 175 160 180 178 190M182 171C201 159 223 165 238 183C217 175 200 180 182 190" />
          ...
          <Transition name="golden-reading">
            <article v-if="currentDeclaration" class="golden-crack-reading" role="status">
...
/* Mi Balance florece como un cerezo: cada flor conserva un gasto real. */
.detail-balance{--cherry-petal:#e9b6c6;--cherry-petal-light:#fff0f2;--cherry-ink:#704958}
.detail-balance .balance-reading::before{display:none}
.balance-reading{display:grid;grid-template-columns:minmax(0,1fr) auto;align-items:end;gap:2rem;padding:.25rem 0 1.5rem;border-bottom:1px solid rgba(201,168,106,.2);text-align:left}
.balance-reading>.balance-heading{display:block;margin:0}.balance-reading>.balance-heading>button{min-height:0;padding:0;border:0;border-radius:0;background:transparent;color:#f4efe5;text-align:left;cursor:pointer}.balance-heading>button span{display:block;color:#c0b8ac;font:650 .68rem/1 system-ui,sans-serif;letter-spacing:.11em;text-transform:uppercase}.balance-heading>button strong{display:block;margin-top:.55rem;color:#ead6a7;font-size:clamp(2.6rem,6vw,4.9rem);font-weight:200;line-height:1;letter-spacing:-.03em;animation:balance-value-breathe 4.6s ease-in-out infinite}.balance-heading>p{max-width:38rem;margin:1rem 0 0;color:#bdb5aa;font-size:.92rem;line-height:1.65}
.balance-reading>.balance-actions{display:flex;justify-content:flex-end;gap:.65rem;margin:0}.balance-actions button,.segmented-choice button{min-height:44px;border:1px solid rgba(201,168,106,.28);border-radius:14px;background:transparent;color:#d8d1c6;padding:.65rem 1rem;font:600 .78rem/1 system-ui,sans-serif;cursor:pointer}.balance-actions button:is(:hover,:focus-visible),.balance-actions button.active,.segmented-choice button.active{border-color:#c9a86a;background:rgba(201,168,106,.13);color:#f3dfb2}.balance-actions button:focus-visible,.balance-blossom:focus-visible,.balance-flower-card>button:focus-visible{outline:2px solid #f0cdd6;outline-offset:3px}
.balance-grove{position:relative;isolation:isolate;width:min(100%,70rem);aspect-ratio:720/560;margin:clamp(1rem,3vw,2rem) auto 0;overflow:hidden;background:radial-gradient(circle at 76% 16%,rgba(233,182,198,.1),transparent 18%),radial-gradient(ellipse at 50% 100%,rgba(201,168,106,.08),transparent 46%)}
.cherry-tree{position:absolute;inset:0;width:100%;height:100%;overflow:visible}.cherry-ground{fill:rgba(0,0,0,.38);filter:blur(8px)}.cherry-trunk{fill:url(#balance-trunk);stroke:rgba(234,214,167,.18);stroke-width:1}.cherry-trunk-light{fill:rgba(244,218,198,.1)}.cherry-branch,.cherry-twig{fill:none;stroke:url(#balance-branch);stroke-linecap:round;stroke-linejoin:round}.cherry-branch{stroke-width:13}.cherry-twig{stroke-width:5}.branch-two,.branch-five{stroke-width:9}.branch-four{stroke-width:11}
.cherry-moon{position:absolute;z-index:-1;right:8%;top:9%;width:clamp(4rem,10vw,7rem);aspect-ratio:1;border:1px solid rgba(234,214,167,.13);border-radius:50%;background:radial-gradient(circle at 38% 36%,rgba(255,244,201,.16),rgba(201,168,106,.03) 50%,transparent 71%);box-shadow:0 20px 48px rgba(0,0,0,.22)}
.balance-blossom{position:absolute;z-index:2;display:grid;width:46px;height:46px;place-items:center;padding:0;border:0;background:transparent;cursor:pointer;transform:translate(-50%,-50%);animation:balance-blossom-open 560ms var(--blossom-delay) cubic-bezier(.16,1,.3,1) both}.balance-blossom-shape{position:relative;display:block;width:30px;height:30px;transform:rotate(var(--blossom-rotation)) scale(var(--blossom-scale));transition:filter 180ms ease,transform 260ms cubic-bezier(.16,1,.3,1)}.balance-blossom-shape i{position:absolute;left:9px;top:1px;width:12px;height:17px;border-radius:70% 70% 58% 58%;background:linear-gradient(155deg,var(--cherry-petal-light),var(--cherry-petal) 58%,#b9788d);box-shadow:0 5px 12px rgba(32,11,20,.2);transform:rotate(calc((var(--petal) - 1) * 72deg)) translateY(-6px);transform-origin:6px 14px}.balance-blossom-shape b{position:absolute;left:12px;top:12px;width:6px;height:6px;border-radius:50%;background:#d6aa68;box-shadow:0 2px 7px rgba(214,170,104,.45)}.balance-blossom:is(:hover,.active) .balance-blossom-shape{filter:brightness(1.12) drop-shadow(0 8px 13px rgba(233,182,198,.24));transform:rotate(var(--blossom-rotation)) scale(calc(var(--blossom-scale) * 1.2))}.balance-blossom.active{z-index:5}
.balance-flower-card{position:absolute;z-index:6;right:6%;bottom:10%;display:grid;width:min(17rem,48%);box-sizing:border-box;gap:.5rem;padding:1.15rem 1.2rem 1.25rem;border:1px solid rgba(233,182,198,.3);border-radius:14px;background:radial-gradient(circle at 100% 0,rgba(233,182,198,.14),transparent 42%),#0d121b;box-shadow:0 22px 56px rgba(0,0,0,.38)}.balance-flower-card>button{position:absolute;right:.45rem;top:.4rem;display:grid;width:44px;height:44px;place-items:center;border:0;background:transparent;color:#d8d1c6;cursor:pointer}.balance-flower-card>button svg{width:1rem}.balance-flower-card>span{padding-right:2rem;color:#d8a8b7;font:650 .65rem/1.2 system-ui,sans-serif;letter-spacing:.1em;text-transform:uppercase}.balance-flower-card>strong{color:#f4efe5;font-size:clamp(1.6rem,4vw,2.25rem);font-weight:300;line-height:1.05}.balance-flower-card>p{margin:.25rem 0 0;color:#d1c9be;font-size:.88rem;line-height:1.55}.balance-flower-card>time{color:#9e968c;font:500 .68rem/1.3 system-ui,sans-serif}
.balance-grove-empty{position:absolute;z-index:2;left:7%;bottom:12%;display:grid;max-width:19rem;gap:.65rem;color:#bdb4a9}.balance-grove-empty svg{width:1.7rem;color:#d9a9b8}.balance-grove-empty p{margin:0;font-style:italic;line-height:1.6}.balance-grove-note{position:absolute;right:4%;bottom:2%;margin:0;color:#918980;font:500 .68rem/1.3 system-ui,sans-serif;letter-spacing:.04em}.balance-flower-reading-enter-active,.balance-flower-reading-leave-active{transition:opacity 180ms ease,filter 240ms ease,transform 340ms cubic-bezier(.16,1,.3,1)}.balance-flower-reading-enter-from,.balance-flower-reading-leave-to{opacity:0;filter:blur(4px);transform:translateY(.8rem) scale(.97)}
@keyframes balance-blossom-open{from{opacity:.2;filter:blur(5px);transform:translate(-50%,-50%) scale(.35) rotate(-12deg)}to{opacity:1;filter:blur(0);transform:translate(-50%,-50%) scale(1) rotate(0)}}
@media(max-width:760px){.balance-reading{grid-template-columns:minmax(0,1fr);gap:1.25rem}.balance-reading>.balance-actions{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));justify-content:stretch}.balance-actions button{width:100%}.balance-grove{aspect-ratio:1/1;min-height:31rem}.cherry-tree{left:-12%;width:124%;max-width:none}.balance-blossom{width:44px;height:44px}.balance-flower-card{position:absolute;z-index:6;right:1rem;bottom:7rem;left:1rem;width:auto;max-width:calc(100% - 2rem)}.balance-grove-empty{left:1.25rem;bottom:3.2rem}.balance-grove-note{right:1rem}.balance-lists{margin-top:1.5rem}}
@media(max-width:420px){.balance-heading>button strong{font-size:2.65rem}.balance-heading>p{font-size:.84rem}.balance-grove{min-height:29rem}.balance-reading>.balance-actions{grid-template-columns:1fr}.balance-blossom-shape{transform:rotate(var(--blossom-rotation)) scale(calc(var(--blossom-scale) * .9))}.balance-blossom:is(:hover,.active) .balance-blossom-shape{transform:rotate(var(--blossom-rotation)) scale(var(--blossom-scale))}.balance-grove-empty{max-width:15rem}.balance-grove-note{font-size:.6rem}}
@media(prefers-reduced-motion:reduce){.balance-blossom,.balance-heading>button strong{animation:none}.balance-flower-reading-enter-active,.balance-flower-reading-leave-active{transition-duration:1ms}}

/* El espacio funcional conserva la materia ritual de cada eje. */
.tw-workspace{--workspace-accent:#c9a86a;position:relative;isolation:isolate;animation:workspace-unveil 560ms cubic-bezier(.16,1,.3,1) both}.detail-world-decretos,.detail-world-vinculos,.detail-nucleo{--workspace-accent:#8173b7}.detail-world-travesias,.detail-world-cuidado{--workspace-accent:#7da797}.workspace-aura{position:absolute;z-index:-1;inset:-4rem -3rem auto;height:28rem;pointer-events:none;background:radial-gradient(ellipse at 72% 8%,color-mix(in srgb,var(--workspace-accent) 14%,transparent),transparent 56%);mask-image:linear-gradient(#000,transparent 88%);animation:aura-drift 9s ease-in-out infinite alternate}.workspace-header{position:relative}.workspace-title{display:grid;min-width:0;flex:1;grid-template-columns:minmax(0,1fr) 4.5rem;align-items:start;gap:1.5rem}.workspace-title::after{content:'';position:absolute;right:0;bottom:-1rem;width:min(58%,28rem);height:1px;background:linear-gradient(90deg,transparent,var(--workspace-accent),transparent);transform-origin:right;animation:ritual-thread 700ms 100ms cubic-bezier(.16,1,.3,1) both}.workspace-sigil{position:relative;display:grid;width:4.25rem;aspect-ratio:1;place-items:center;border:1px solid color-mix(in srgb,var(--workspace-accent) 44%,transparent);border-radius:50%;color:var(--workspace-accent);box-shadow:0 18px 40px rgba(0,0,0,.25)}.workspace-sigil::before{content:'';position:absolute;inset:-8px;border:1px solid color-mix(in srgb,var(--workspace-accent) 22%,transparent);border-radius:50%;border-block-color:transparent;animation:sigil-orbit 8s linear infinite}.workspace-sigil svg{width:1.7rem}.ritual-form{position:relative;overflow:hidden;border:0;border-block:1px solid color-mix(in srgb,var(--workspace-accent) 35%,transparent);border-radius:0;background:radial-gradient(circle at 6% 0,color-mix(in srgb,var(--workspace-accent) 11%,transparent),transparent 44%);box-shadow:none;clip-path:polygon(0 0,calc(100% - 16px) 0,100% 16px,100% 100%,16px 100%,0 calc(100% - 16px))}.ritual-form::before{content:'';position:absolute;inset:0 auto 0 0;width:1px;background:linear-gradient(transparent,var(--workspace-accent),transparent);animation:form-current 4.8s ease-in-out infinite}.ritual-form :is(input,select,textarea){transition:border-color 180ms ease,background-color 180ms ease,box-shadow 220ms ease}.ritual-form :is(input,select,textarea):focus{border-color:var(--workspace-accent);background:#0b1018;box-shadow:0 12px 34px color-mix(in srgb,var(--workspace-accent) 8%,transparent)}.workspace-primary{position:relative;overflow:hidden;border-radius:0;clip-path:polygon(10px 0,100% 0,100% calc(100% - 10px),calc(100% - 10px) 100%,0 100%,0 10px);transition:background-color 180ms ease,box-shadow 240ms ease,filter 120ms ease}.workspace-primary::after{content:'';position:absolute;inset:0;background:linear-gradient(110deg,transparent 30%,rgba(255,248,224,.18) 50%,transparent 70%);transform:translateX(-120%);transition:transform 520ms cubic-bezier(.16,1,.3,1)}.workspace-primary:hover::after,.workspace-primary:focus-visible::after{transform:translateX(120%)}.workspace-primary:hover{background:color-mix(in srgb,var(--workspace-accent) 20%,transparent);box-shadow:0 14px 34px color-mix(in srgb,var(--workspace-accent) 12%,rgba(0,0,0,.2))}.workspace-record{position:relative;padding-inline-start:1.35rem;animation:record-materialize 420ms cubic-bezier(.16,1,.3,1) both}.workspace-record::before{content:'';position:absolute;left:.15rem;top:50%;width:5px;height:5px;border-radius:50%;background:var(--workspace-accent);box-shadow:0 0 14px color-mix(in srgb,var(--workspace-accent) 70%,transparent);transform:translateY(-50%);transition:box-shadow 180ms ease,transform 180ms ease}.workspace-record:hover::before{box-shadow:0 0 22px var(--workspace-accent);transform:translateY(-50%) scale(1.45)}.balance-reading{position:relative;padding-block:2.5rem}.balance-reading::before{content:'';position:absolute;z-index:-1;left:50%;top:50%;width:min(29rem,88vw);aspect-ratio:1;border:1px solid rgba(201,168,106,.19);border-radius:50%;border-inline-color:rgba(129,115,183,.32);transform:translate(-50%,-50%) rotateX(64deg);animation:balance-celestial-turn 14s linear infinite}.balance-reading>button strong{animation:balance-value-breathe 4.6s ease-in-out infinite}.thought-cloth{background-size:135% 135%;animation:cloth-current 9s ease-in-out infinite alternate}.thought-cloth>button span{animation:thought-pulse 3.4s ease-in-out infinite}.thought-cloth>button:nth-of-type(2n) span{animation-delay:-1.4s}.golden-sculpture{background-size:140% 140%;animation:resin-current 8s ease-in-out infinite alternate}.golden-sculpture>.resin-rift{animation:rift-light 4.8s ease-in-out infinite;animation-delay:calc(var(--rift-index) * -.38s)}.golden-sculpture>button span{animation:golden-node-pulse 3.2s ease-in-out infinite}.umbral-workspace{position:relative}.umbral-workspace::before{content:'';position:absolute;z-index:-1;left:50%;top:18rem;width:42rem;max-width:92vw;aspect-ratio:1;border:1px solid rgba(201,168,106,.1);border-radius:50%;transform:translate(-50%,-50%) rotateX(70deg);animation:umbral-orbit 22s linear infinite}.intention-row{animation:record-materialize 360ms cubic-bezier(.16,1,.3,1) both}
@keyframes workspace-unveil{from{opacity:.55;filter:blur(8px);clip-path:inset(0 0 90% 0)}to{opacity:1;filter:blur(0);clip-path:inset(0)}}
...
@keyframes balance-celestial-turn{to{transform:translate(-50%,-50%) rotateX(64deg) rotateZ(360deg)}}
@keyframes balance-value-breathe{0%,100%{filter:brightness(.9);text-shadow:0 8px 28px rgba(201,168,106,.08)}50%{filter:brightness(1.1);text-shadow:0 14px 38px rgba(201,168,106,.26)}}
...
@media(max-width:760px){.tw-workspace{width:100%;max-width:100%;min-height:0;overflow-x:clip}.workspace-header{display:block}.workspace-back{margin-bottom:1.25rem}.workspace-title{grid-template-columns:minmax(0,1fr) 3.5rem;gap:1rem}.workspace-sigil{width:3.25rem}.workspace-grid,.balance-lists,.umbral-workspace{grid-template-columns:minmax(0,1fr)}.workspace-grid>*,.balance-lists>*,.umbral-workspace>*{min-width:0}.note-grid{grid-template-columns:repeat(4,minmax(0,1fr))}.thought-cloth{width:100%;min-height:min(20rem,68svh)}.balance-reading>div{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}.golden-sculpture{width:min(25rem,86vw)}.golden-sculpture>button em{display:none}.golden-selected-copy{display:block;max-width:32rem;margin:1rem auto;color:#ead6a7;overflow-wrap:anywhere;font-style:italic;line-height:1.6}.ritual-form,.nucleus-entry,.golden-entry{width:100%;max-width:100%;box-sizing:border-box}.workspace-record>div{min-width:0}}
@media(max-width:420px){.workspace-aura{inset:-2rem 0 auto;width:100%}.workspace-header h1{max-width:100%;font-size:clamp(2.15rem,12vw,3rem);overflow-wrap:anywhere}.workspace-title{grid-template-columns:minmax(0,1fr) 2.85rem;gap:.65rem}.workspace-sigil{width:2.7rem}.workspace-sigil svg{width:1.2rem}.ritual-form{padding:1rem}.workspace-record{align-items:flex-start;flex-wrap:wrap}.workspace-record>button{width:100%}.balance-reading{padding-block:1.5rem}.balance-reading>div{grid-template-columns:1fr}.balance-lists{gap:1.25rem}.goal-colors{display:flex;flex-wrap:wrap;gap:.65rem}.goal-colors button{margin:0}.note-grid{grid-template-columns:repeat(4,minmax(0,1fr))}.note-grid button{padding-inline:.25rem}.golden-sculpture{width:min(21rem,calc(100vw - 2.5rem))}.thought-reading{max-width:100%;box-sizing:border-box}.workspace-primary{width:100%}}
@media(max-width:760px) and (max-height:520px){.golden-sculpture{width:min(16rem,48svh)}.thought-cloth{min-height:58svh}.workspace-header{margin-bottom:1.25rem}.workspace-header h1{font-size:2.25rem}.balance-reading{padding-block:.75rem}}
...
.detail-edad-dorada .daruma-stage{position:relative;isolation:isolate;width:min(100%,36rem);aspect-ratio:360/450;margin-inline:auto;overflow:visible;border-radius:0;background:none;box-shadow:none;animation:none}
...
.daruma-brow,.daruma-moustache{fill:none;stroke:#0a0c11;stroke-width:8;stroke-linecap:round;stroke-linejoin:round}
...
.golden-crack-reading{position:relative;width:min(100%,31rem);box-sizing:border-box;margin:.4rem auto 0;padding:1rem 1.1rem 1.15rem;border:1px solid rgba(201,168,106,.32);border-radius:14px;background:radial-gradient(circle at 8% 0,rgba(201,168,106,.1),transparent 44%),#0d121b;box-shadow:0 18px 48px rgba(0,0,0,.32)}
.golden-crack-reading::before{content:'';position:absolute;left:1.15rem;top:-.55rem;width:1px;height:.55rem;background:#c9a86a}
...
@media(max-width:760px){.golden-presence{grid-template-columns:minmax(0,1fr);gap:1.2rem}.detail-edad-dorada .daruma-stage{width:min(100%,28rem)}.golden-crack-reading{position:fixed;z-index:80;inset:auto 1rem calc(5.9rem + env(safe-area-inset-bottom));width:auto;margin:0}.golden-practice{margin-top:.8rem}.detail-edad-dorada .golden-copy{text-align:center}.detail-edad-dorada .golden-copy span{margin-inline:auto}.detail-edad-dorada .golden-entry .workspace-primary{width:100%}.detail-edad-dorada.is-contemplative .golden-workspace{padding:1rem}.detail-edad-dorada .contemplation-exit{right:1rem;top:1rem}}
...
.constellation-copy{max-width:39rem;margin:0 0 1rem;color:#c8c0d2;font-style:italic;line-height:1.6}
...
@media(prefers-reduced-motion:reduce){.tw-workspace{animation:workspace-appear 160ms ease-out both}.workspace-aura,.workspace-sigil::before,.ritual-form::before,.balance-reading::before,.balance-reading>button strong,.thought-cloth,.thought-cloth>button span,.plasma-pool,.golden-sculpture,.golden-sculpture>.resin-rift,.golden-sculpture>button span,.umbral-workspace::before{animation:none}.workspace-record,.intention-row{animation:none}.thought-float-enter-active,.thought-float-leave-active,.thought-float-enter-active .thought-reading,.thought-float-leave-active .thought-reading{transition-duration:1ms}.tw-workspace *{scroll-behavior:auto!important;transition-duration:120ms!important}}@keyframes workspace-appear{from{opacity:.72}to{opacity:1}}
</style>
