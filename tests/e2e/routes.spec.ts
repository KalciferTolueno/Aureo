...
test.beforeEach(async ({ page }) => {
  await page.addInitScript((value) => {
    ...
    if (!localStorage.getItem('aureo_umbral_momento_apertura')) localStorage.setItem('aureo_umbral_momento_apertura', JSON.stringify(today))
  }, profile)
})
...
test('abre la superficie Tailwind sin errores visibles', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: /El día es tuyo|La tarde es tuya|La noche es tuya/ })).toBeVisible()
...
test('completa el onboarding ritual y deja el correo solo en este dispositivo', async ({ page }) => {
  test.setTimeout(60_000)
  await page.emulateMedia({ reducedMotion: 'reduce' })
  await page.addInitScript(() => { localStorage.removeItem('aureo_configuracion') })
  await page.goto('/#/')
  await expect(page.getByRole('button', { name: 'Entrar' })).toBeVisible()
  await page.getByRole('button', { name: 'Entrar' }).click()
  await page.getByRole('button', { name: 'Es mío y solo mío' }).click()
  await page.getByPlaceholder('tu nombre íntimo').fill('Melita')
  await page.getByRole('button', { name: 'Así me llamo' }).click()
  await page.getByPlaceholder('22').fill('21')
  await page.getByPlaceholder('06').fill('3')
  await page.getByPlaceholder('1990').fill('1990')
  await page.getByRole('button', { name: 'Este es mi origen' }).click()
  await page.getByRole('button', { name: 'Este es mi origen' }).click()
  await page.getByRole('button', { name: 'Reconozco lo que soy' }).click()
  await page.getByLabel('Correo').fill('melita@example.com')
  await page.getByRole('button', { name: 'Así me encuentran' }).click()
  for (const note of ['Do', 'Re', 'Mi']) await page.getByRole('button', { name: note, exact: true }).click()
  await page.getByRole('button', { name: 'Este es mi universo' }).click()
  await expect(page.locator('main.tailwind-lab')).toBeVisible()
  await expect.poll(async () => {
    const stored = await page.evaluate(() => JSON.parse(localStorage.getItem('aureo_configuracion') ?? '{}') as { onboarding_completo?: boolean; email?: string })
    return stored.onboarding_completo === true && stored.email === 'melita@example.com'
  }).toBe(true)
  await expect(page.locator('input[type="email"]')).toHaveCount(0)
})
...
test('Tu mazo diario muestra solo los últimos 7 días y conserva el archivo', async ({ page }) => {
  const names = ['El Loco', 'El Mago', 'La Sacerdotisa', 'La Emperatriz', 'El Emperador', 'El Hierofante', 'Los Enamorados', 'El Carro', 'La Fuerza', 'La Rueda']
  await page.addInitScript((payload: string[]) => {
    const items = payload.map((nombre, index) => {
      const date = new Date()
      date.setDate(date.getDate() - (payload.length - 1 - index))
      const fecha = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`
      return { id: `arcano-${fecha}`, fecha, nombre, fecha_creacion: date.toISOString() }
    })
    localStorage.setItem('aureo_umbral_arcanos', JSON.stringify(items))
  }, names)
  await page.goto('/#/?axis=umbral')
  await page.getByRole('button', { name: /Abrir el mazo/ }).click()
  const deckDialog = page.getByRole('dialog', { name: 'Tu mazo diario' })
  await expect(deckDialog).toBeVisible()
  await expect(deckDialog).toHaveAttribute('id', 'umbral-arcana-history')
  await expect(page.locator('.umbral-threshold-notes #umbral-arcana-history')).toHaveCount(0)
  await expect(deckDialog.locator('ol li')).toHaveCount(7)
  await expect(page.getByRole('heading', { name: 'Tu mazo diario' })).toBeVisible()
  await expect(page.getByText('El Loco', { exact: true })).toHaveCount(0)
  await expect(page.getByText('La Rueda', { exact: true })).toBeVisible()
  const stored = await page.evaluate(() => JSON.parse(localStorage.getItem('aureo_umbral_arcanos') ?? '[]') as { nombre: string }[])
  expect(stored.map((item) => item.nombre)).toEqual(expect.arrayContaining(names))
  expect(stored.length).toBeGreaterThanOrEqual(names.length)
})
...
test('conecta la portada Tailwind con las funciones reales de cada eje', async ({ page }, testInfo) => {
  ...
  await expect(deckDialog).toBeVisible()
  await expect(page.getByRole('heading', { name: 'Tu mazo diario' })).toBeVisible()
  await deckDialog.getByRole('button', { name: 'Cerrar el mazo' }).click()
  await expect(deckDialog).toHaveCount(0)
  await expect(page.getByText('Vista experimental')).toHaveCount(0)
  await expect(page.getByText(/esta vista solo lee/i)).toHaveCount(0)
  ...
  await page.getByRole('button', { name: 'Editar ingreso base' }).click()
  await page.getByLabel('Mi ingreso base').fill('150000')
  await page.getByRole('button', { name: 'Guardar ingreso base' }).click()
  await expect(page.getByRole('status', { name: 'Guardado' })).toBeVisible()
  ...
  expect(await page.evaluate(() => localStorage.getItem('aureo_balance_ingreso_base'))).toBe('150000')
  await page.getByRole('tab', { name: 'Lo que construyo' }).click()
  ...
  await expect(page.locator('.golden-daruma-hint')).toHaveText('Toca el daruma para contemplar sus grietas')
  await expect(page.getByRole('button', { name: 'Toca el daruma para contemplar sus grietas' })).toBeVisible()
})
...
test('forma y permite recorrer las grietas del Daruma de Edad Dorada', async ({ page }, testInfo) => {
  ...
  for (const declaration of ['Reconozco mi valentía presente', 'Estoy construyendo una vida propia', 'Este momento también es plenitud']) {
    await page.getByLabel('¿Qué reconoces hoy?').fill(declaration)
    await page.getByRole('button', { name: 'Formar una grieta' }).click()
    await expect(page.getByRole('status')).toContainText('Una nueva grieta guarda este momento')
  }
  await expect(page.locator('.golden-daruma-entry-crack')).toHaveCount(3)
  await page.getByRole('button', { name: 'Toca el daruma para contemplar sus grietas' }).click()
  await expect(page).toHaveURL(/axis=edad-dorada&detail=edad-dorada/)
  await expect(page.getByLabel('¿Qué reconoces hoy?')).toHaveCount(0)
...
test('integra los datos del día en la órbita', async ({ page }) => {
  ...
  await expect(orbit.locator('.umbral-datum-number')).toContainText(/Número/i)
  await expect(page.locator('.umbral-datum-sign')).toHaveCount(0)
  await expect(orbit.locator('.umbral-datum-arcana')).toContainText(/Arcano/i)
  const numberBox = await orbit.locator('.umbral-datum-number').boundingBox()
  const arcanaBox = await orbit.locator('.umbral-datum-arcana').boundingBox()
  expect(numberBox && arcanaBox).toBeTruthy()
  expect(arcanaBox!.x).toBeGreaterThan(numberBox!.x)
  await expect(page.getByRole('region', { name: 'Carta del día' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Modo día' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Modo noche' })).toBeVisible()
  await page.getByRole('button', { name: 'Modo noche' }).click()
  await expect(page.locator('main.tailwind-lab')).toHaveAttribute('data-lumen', 'noche')
  await expect.poll(() => page.locator('main.tailwind-lab').evaluate((element) => {
    const style = getComputedStyle(element)
    return style.backgroundColor === 'rgb(8, 11, 17)' || /8,\s*11,\s*17|#080b11/i.test(style.background)
  })).toBe(true)
  await page.getByRole('button', { name: 'Modo día' }).click()
  await expect(page.locator('main.tailwind-lab')).toHaveAttribute('data-lumen', 'dia')
  await expect.poll(() => page.locator('main.tailwind-lab').evaluate((element) => {
    const style = getComputedStyle(element)
    return style.backgroundColor === 'rgb(8, 11, 17)' || /8,\s*11,\s*17|#080b11/i.test(style.background)
  })).toBe(true)
  ...
  await expect(page.getByRole('button', { name: 'Capturar un destello' })).toBeVisible()
  await expect(page.locator('.lab-orbit')).toHaveCount(3)
  await expect(page.locator('.module-card.featured')).toHaveCount(0)
  await page.locator('.umbral-number-trigger').click()
  await expect(page.locator('.today-carta-reading')).toBeVisible()
  await expect(page.locator('.today-carta-phrase')).toBeVisible()
  await expect(page.getByText('Tu sello', { exact: true })).toBeVisible()
  await page.locator('nav:visible').getByRole('button', { name: 'Núcleo', exact: true }).click()
...
test('el diálogo conserva un recorrido de teclado accesible', async ({ page }) => {
  ...
  const goal = page.getByRole('dialog', { name: 'Crear una meta' })
  await expect(goal).toBeVisible()
  await expect(goal.getByLabel('Meta', { exact: true })).toBeVisible()
  await expect(goal.getByText('$', { exact: true })).toBeVisible()
  await page.keyboard.press('Escape')
